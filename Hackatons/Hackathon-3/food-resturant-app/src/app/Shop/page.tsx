...
export default function Shop(){
    ...
    //const [sandwich, setSandwich] = useState(false);
     async function fetchData(){
        const response = await client.fetch(`*[_type == "food"][0..8]{_id, name, price, originalPrice,
    ...
    const handleSandwich = async (e:any) =>{
        if(e.target.checked == true){
            const data = await client.fetch(`*[_type == "food" && category=="Sandwich"]{_id, name, price, originalPrice,
                "imageUrl": image.asset->url, "slug":slug.current}`);
            setFoods(data);
        }
    ...
    const handleBurger = async () =>{
        const data = await client.fetch(`*[_type == "food" && category == "Burger"]{_id, name, price, originalPrice,
            "imageUrl": image.asset->url, "slug":slug.current}`);
        setFoods(data);
    }
    const handleChicken = async () =>{
        const data = await client.fetch(`*[_type == "food" && category == "Burger"]{_id, name, price, originalPrice,
            "imageUrl": image.asset->url, "slug":slug.current}`);
        setFoods(data);
    }
            ...
            <div className="grid place-content-center my-[120px]">
                ...
                <div className="mt-6 flex gap-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center">
                        {foods.map((items:any)=>{
                               return <Link href={`foods/${items.slug}`}key={items._id}><CardProducts image={items.imageUrl}
                               name={items.name}
                                actualRate={items.originalPrice}
                                discountRate={items.price}/></Link>
                        })}
